...
| `PeekStreamEntryMessage`        | `0x0805` | Yes         | No       | Get the value of the given stream at the given index. If the value is not present, this entry is completed immediately with empty completion. If index == `-1`, the latest index will be fetched. Note: the completion value for this message is a protobuf of type `StreamEntry`. |
| `NextStreamEntryMessage`        | `0x0805` | Yes         | No       | Get or wait the value of the given stream at the given index. If the value is not present yet, this entry will block waiting for the value. If index == `-1`, the latest index will be fetched. Note: the completion value for this message is a protobuf of type `StreamEntry`.   |
| `AppendStreamEntryMessage`      | `0x0806` | No          | No       | Append a new element to a stream. If the given stream doesn't exist, this operation creates the stream.                                                                                                                                                                            |
| `SetStreamEntryMessage`         | `0x0807` | Yes         | No       | Set an element for a given index in the stream. If a value is already set for the given index, or the index is not == current stream length, the operation fails. If the given stream doesn't exist, this operation creates the stream.                                            |
